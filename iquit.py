    ...
    try:
        while True:
            ...
            if touch_start_time is not None:
                ...
                if time.time() - touch_start_time >= 1.5:
                    print("Activated")
                    prediction = predict()
                    print(prediction)
...
def motors(quad):
    ...
    def rotate_servo():
        ...
        # Open the servo
        for i in range(0,29):  # Adjusted angle range from 190 to 210 degrees
        ...
        # Close the servo
        for i in range(180,210):  # Adjusted angle range from 0 to 60 degrees
            servo.value = math.sin(math.radians(i))
            sleep(0.01)
    ...
    try:
        ...
        duration_values = {1: 0.043, 2: 0.15, 3: 0.29, 4: 0.45} #1:paper, 2: metal, 3: plastic, 4: general

        # Final duration values for each quadrant
        final_duration_values = {1: 0.3, 2: 0.15, 3: 0.13, 4: 0.2}

        material_quadrant = quad  # Change this to the desired material quadrant (1, 2, 3, or 4)
        
        print(quad)
        # Get the duration for the current quadrant
        duration_stepper = duration_values.get(material_quadrant)
        final_duration_stepper = final_duration_values.get(material_quadrant)
        print(duration_stepper)
        print(final_duration_stepper)
...
def predict():
    model_path = 'best_float32.tflite' #directory for the mlbins
